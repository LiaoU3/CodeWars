...
# For example, a "5" can only count as part of a triplet (contributing to the 500 points) or 
...
def score(dice):
    ...
    for number in dice:
        count[number-1] += 1
    return (count[0]//3) * 1000 + (count[0]%3)* 100 + (count[1]//3) * 200+ (count[2]//3) * 300 + (count[3]//3) * 400 + (count[4]//3) * 500 + (count[4]%3)*50 + (count[5]//3) * 600
